...
MedTermQuest is an adaptive medical terminology learning system. Instead of quizzing you randomly, it maintains a **Bayesian belief distribution** over your knowledge of 47 medical terms across 13 domains, then uses **Shannon entropy maximization** to greedily pick whichever question will reveal the most about your knowledge state.

The system treats your medical knowledge as a probability distribution and applies information theory to learn *you* as efficiently as possible — converging on your true knowledge state in the fewest possible questions.
...
Given a medical lexicon L of n terms, determine which subset S ⊆ L a user knows, subject to the constraint that each observed answer is consistent with their true knowledge state.

This is a **constraint satisfaction problem** over a probabilistic belief space.

### The Solution (Greedy Entropy Maximization)

Maintain a belief vector **B ∈ [0,1]ⁿ** where `B[i] = P(user knows term i)`.
...
Next question = argmax_{term ∉ asked} EIG(term)
```

where `H(B) = -Σ p·log₂(p) + (1-p)·log₂(1-p)` is average binary entropy.
...
Each term includes: definition, etymology/root, mnemonic, related terms, diagnostic constraints.
...
| POST | `/quiz/answer` | Submit answer → feedback + next question |
...
Just open `frontend/index.html` directly in a browser. The frontend works completely standalone using browser `localStorage` — the full entropy algorithm runs client-side in JavaScript. No server, no installation needed.
...
**Why not a React app?** The frontend is a single `index.html` with no build step — zero dependencies, opens in any browser. The full entropy algorithm is implemented in both Python (backend) and JavaScript (frontend), so the app works completely offline.
...
**Why not a larger question bank?** Each term requires a definition, etymology, mnemonic, and related terms — quality over quantity. The entropy algorithm extracts maximum information from each answer, so 47 high-quality terms outperform 500 low-quality ones.

**Why greedy and not optimal?** Optimal adaptive testing (via POMDP) is NP-hard for n > ~20 terms. Greedy EIG is O(n) per step, produces near-optimal results in practice, and is theoretically sound — provably convergent and monotonically entropy-reducing.
